...
const sizeByPrice = async (sizeId, campaignId) => {
  ...
  try {
    const res = await axiosInstance.get(
      `/campaigns/products/${campaignId}/${route.params.productId}/${sizeId}`
    );
          ...
          <div class="col-lg-6">
            <div class="details-content">
              ...
              <div class="details-meta">
                <p>
                  SKU:<span>1234567{{ campaignProduct?.sku }}</span>
                </p>
                <p>
                  BRAND:<a href="#">{{ campaignProduct?.brand_id }}</a>
                </p>
              </div>
              <template v-if="campaignVariationProduct.length > 0">
                <h3 class="details-price" v-if="!productPrices">
                  <span
                    v-for="(productsPrice, index) in campaignVariationProduct"
                    :key="index"
                  >
                    <span v-if="productsPrice.pivot.is_default == 1"
                      ><del>{{
                        $filters.currencySymbol(productsPrice.pivot.mrp)
                      }}</del>
                      <span
                        >{{
                          $filters.currencySymbol(
                            productsPrice.pivot.offer_price
                          )
                        }}<small>/per kilo</small></span
                      >
                    </span>
                  </span>
                </h3>
                <h3 class="details-price" v-else>
                  <span
                    ><del>{{ $filters.currencySymbol(productPrices.mrp) }}</del>
                    <span
                      >{{ $filters.currencySymbol(productPrices.offer_price)
                      }}<small>/per kilo</small></span
                    ></span
                  >
                </h3>
              ...
              <template v-else>
                <h3 class="details-price">
                  ...
                  <span
                    >{{ campaignProduct?.pivot?.offer_price
                    }}<small>/per kilo</small></span
                  >
                </h3>
              </template>
              <div class="details-list-group">
                ...
                <ul class="details-tag-list">
                  <li
                    v-for="(
                      size, index
                    ) in campaignProduct?.campaign_product_prices"
                    :key="index"
                  >
                    <a
                      href="#"
                      @click.prevent="
                        sizeByPrice(size.id, size.pivot.campaign_id)
                      "
                      :class="{ selectedSizeColor: size.id === selectedSize }"
                      >{{ size.name }}</a
                    >
                  </li>
                </ul>
              ...
              <p
                class="details-desc"
                v-html="campaignProduct?.short_description"
              ></p>
              <div class="details-list-group">
                ...
                <ul class="details-share-list">
                  <li v-for="(socialShare, index) in socialShares" :key="index">
                    <a
                      :href="socialURL(socialShare.type, socialShare.contact)"
                      title=""
              ...
              <div
                class="details-add-group"
                v-if="campaignVariationProduct.length > 0"
              >
                <div class="row">
                  <span
